...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t1,) = ctx.saved_values
        one_minus_sigma = grad_output.f.add_zip(grad_output.f.neg_map(grad_output.f.sigmoid_map(t1)), tensor([1]))
        return grad_output.f.mul_zip(grad_output, grad_output.f.mul_zip(grad_output.f.sigmoid_map(t1), one_minus_sigma))
...
class All(Function):
    ...
    def forward(ctx: Context, a: Tensor, dim: Tensor) -> Tensor:
        if dim is not None:
            ...
            return a.f.mul_reduce(a.contiguous().view(int(np.prod(a.shape))), 0)
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        return grad_output._new(grad_output._tensor.permute(*ret)), 0.0
...
def zeros(shape: UserShape, backend: TensorBackend = SimpleBackend) -> Tensor:
    ...
    # )
    return minitorch.Tensor.make([0] * int(np.prod(shape)), shape, backend=backend)
