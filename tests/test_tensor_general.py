...
def test_bmm(backend: str, data: DataObject) -> None:
    ...
    # q = Tensor.make([0, 0, 0, -25], shape=(2, 2), backend=shared[backend]).contiguous().view(1, 2, 2, 1)
    # w = Tensor.make([0, 0, 0, 100], shape=(2, 2), backend=shared[backend]).contiguous().view(1, 1, 2, 2)
    # res = (q * w).sum(2).view(1, 2, 2)
    # print(res)
    # assert False
