...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert sigmoid(a) >= -1e-9 and sigmoid(a) <= 1. + 1e-9
    assert is_close(1 - sigmoid(a), sigmoid(-a))
    assert is_close(0.5, sigmoid(0))
    assert sigmoid(a) * (1 - sigmoid(a)) > -1e-9
...
def test_symmetric() -> None:
    ...
    """
    assert mul(1337, 7331) == mul(7331, 1337)
...
def test_distribute() -> None:
    ...
    """
    assert 63 * (1337 + 7331) == 63 * 1337 + 63 * 7331
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    map_sum = addLists(ls1, ls2)
    simple_sum = [ls1[i] + ls2[i] for i in range(len(ls1))]
    assert map_sum == simple_sum
